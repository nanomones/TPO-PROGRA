...
public final class Reporte {
    ...
    public static void imprimirResumen(Mercado m, Perfil p, Asignacion a) {
        ...
        System.out.printf(java.util.Locale.US,
            "Riesgo (sigma): %.3f (max %.3f)%n", sig, p.getRiesgoMax());

        System.out.println("=== CARTERA ===");
        System.out.printf(java.util.Locale.US, "Presupuesto: %.2f%n", p.getPresupuesto());
        ...
        for (var act : m.activos) {
            double monto = a.getMonto(act.ticker);
            if (monto <= 0) continue;
            double w = monto / p.getPresupuesto() * 100.0;
