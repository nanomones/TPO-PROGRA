...
public final class CargadorDatosJson {
    ...
    public static Mercado cargarMercado(String rutaJson){
        try {
            ...
            for (var fila : mj.correlaciones)
                if (fila.size() != n) throw new IllegalArgumentException("ρ no es n x n");
            ...
            for (int i = 0; i < n; i++)
                for (int j = 0; j < n; j++)
                    rho[i][j] = mj.correlaciones.get(i).get(j);

            // chequeos básicos
            for (int i = 0; i < n; i++) {
                if (Math.abs(rho[i][i] - 1.0) > 1e-9)
                    throw new IllegalArgumentException("ρ[i][i]!=1 en i=" + i);
                for (int j = i + 1; j < n; j++) {
                    if (Math.abs(rho[i][j] - rho[j][i]) > 1e-9)
                        throw new IllegalArgumentException("ρ no simétrica (" + i + "," + j + ")");
                    if (rho[i][j] < -1.0 || rho[i][j] > 1.0)
                        throw new IllegalArgumentException("ρ fuera de [-1,1] (" + i + "," + j + ")");
                }
            }
